...
def verify_hostname(email_addr):
...
def check_existence(email_addr):
    result = ''
    try:
...
def verify_email(email):
    # Check NSLookUp
    nslookup_results = verify_email(email)
    # If NSLookUP exists, verify email
    if nslookup_results is not 0:
        result = check_existence(email)
